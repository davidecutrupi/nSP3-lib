...
namespace solver {
  ...
  template <unsigned int dim, typename number>
  void ZeroModeOperator<dim, number>::initialize(std::shared_ptr<const MatrixFree<dim, number>> data, std::shared_ptr<const MaterialCache<number>> material_cache) {
    clear();
    this->data = data;
  ...
  template <unsigned int dim, typename number>
  void ZeroModeOperator<dim, number>::integrate_cell_physics(FEEval &phi, const unsigned int cell) const {
  ...
  void ZeroModeOperator<dim, number>::integrate_face_physics(FEFaceEval &phi_inner, FEFaceEval &phi_outer) const {
    ...
    const VectorizedArray<number> half_d_in = material_cache->inner_face_diffusion_interior[phi_inner.get_current_cell_index()] * number(0.5);
    const VectorizedArray<number> half_d_out = material_cache->inner_face_diffusion_exterior[phi_outer.get_current_cell_index()] * number(0.5);
