...
namespace data {
  ...
  public:
      ...
      sigma_s(n_materials, n_groups, n_groups)
    {
      ...
      for (const auto &mat : materials) {
        ...
        fill_1d_or_zero(mat, "sigma_f", m, sigma_f);
    ...
    double get_sigma_f(unsigned int m, unsigned int g) const { return sigma_f[m][g]; }
  ...
  private:
    ...
    void fill_1d_or_zero(const json &mat, const std::string &field_name, unsigned int m, Table<2, double> &table) const {
      if (mat.contains(field_name)) {
        fill_1d(mat.at(field_name), m, table);
        return;
      }
