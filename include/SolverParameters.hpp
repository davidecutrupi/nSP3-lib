...
namespace solver {
  ...
  private:
    ...
    void normalize_and_validate() {
      if (benchmark.empty())
        throw std::runtime_error("SolverParameters: Benchmark name must not be empty.");
      if (output_directory.empty())
        output_directory = ".";
      if (fe_type != "DG" && fe_type != "CG")
        throw std::runtime_error("SolverParameters: FE type must be either DG or CG.");
    }
