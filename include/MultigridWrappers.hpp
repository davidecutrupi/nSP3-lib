...
namespace solver {
  ...
  template <unsigned int dim, typename number>
  class MGTransferGlobalCoarsening : public dealii::MGTransferBase<dealii::LinearAlgebra::distributed::Vector<number>> {
  public:
    ...
    void prolongate(const unsigned int to_level, VectorType &dst, const VectorType &src) const override {
      transfer.prolongate(to_level, dst, src);
    }

    void prolongate_and_add(const unsigned int to_level, VectorType &dst, const VectorType &src) const override {
      transfer.prolongate_and_add(to_level, dst, src);
    }

    void restrict_and_add(const unsigned int from_level, VectorType &dst, const VectorType &src) const override {
      transfer.restrict_and_add(from_level, dst, src);
    }

    template <typename VectorType2>
    void copy_to_mg(const dealii::DoFHandler<dim> &dof_handler, dealii::MGLevelObject<VectorType> &dst, const VectorType2 &src) const {
      transfer.copy_to_mg(dof_handler, dst, src);
    }

    template <typename VectorType2>
    void copy_from_mg(const dealii::DoFHandler<dim> &dof_handler, VectorType2 &dst, const dealii::MGLevelObject<VectorType> &src) const {
      transfer.copy_from_mg(dof_handler, dst, src);
    }

    template <typename VectorType2>
    void copy_from_mg_add(const dealii::DoFHandler<dim> &dof_handler, VectorType2 &dst, const dealii::MGLevelObject<VectorType> &src) const {
      transfer.copy_from_mg_add(dof_handler, dst, src);
    }

  private:
    dealii::MGLevelObject<dealii::MGTwoLevelTransfer<dim, VectorType>> two_level_transfers;
    dealii::MGTransferGlobalCoarsening<dim, VectorType> transfer;
  };


  template <unsigned int dim, typename number>
  class MGTransferBlockGlobalCoarsening : public dealii::MGTransferBase<dealii::LinearAlgebra::distributed::BlockVector<number>> {
  public:
    using VectorType = dealii::LinearAlgebra::distributed::Vector<number>;
    using BlockVectorType = dealii::LinearAlgebra::distributed::BlockVector<number>;
    ...
    void build(const dealii::DoFHandler<dim> &active_dof_handler,
      ...
      auto initialize_scalar_vector = [initialize_dof_vector](const unsigned int level, VectorType &vector) {
        BlockVectorType prototype(n_blocks);
        initialize_dof_vector(level, prototype);
      ...
      for (unsigned int level = 0; level <= max_level; ++level) {
        ...
        for (unsigned int block = 0; block < n_blocks; ++block) {
          copy_to_mg_scratch[block][level].reinit(prototype.block(block), true);
          copy_from_mg_scratch[block][level].reinit(prototype.block(block), true);
        }
      }
    }
  ...
  private:
    ...
    MGTransferGlobalCoarsening<dim, number> scalar_transfer;
