...
void DoFInfo::read_dof_indices(
    ...
    const unsigned int cell_number, ConstraintValues<double> &constraint_values,
  ...
  if (store_plain_indices == true) {
    if (cell_number == 0)
      row_starts_plain_indices.resize((row_starts.size() - 1) / n_components +
                                      1);
    ...
    if (cell_has_constraints == true) {
      for (unsigned int i = 0; i < dofs_this_cell; ++i) {
        ...
        if (current_dof < first_owned || current_dof >= last_owned) {
          ...
          current_dof -= first_owned;
        plain_dof_indices.push_back(static_cast<unsigned int>(current_dof));
      }
    }
  }
}
...
template <int dim>
bool DoFInfo::process_hanging_node_constraints(
  ...
  hanging_nodes.update_dof_indices(cell, {}, lexicographic_mapping,
                                   hanging_node_constraint_masks_comp,
                                   refinement_configuration, dof_indices);
...
void DoFInfo::compute_face_index_compression(
  ...
  const unsigned int size_exterior_faces = faces.size();
  ...
  for (unsigned int face = 0; face < faces.size(); ++face) {
    auto face_computation = [&](const DoFAccessIndex face_index,
      ...
      for (unsigned int v = 0;
           v < length && cell_indices_face[v] != numbers::invalid_unsigned_int;
           ++v) {
        ...
        if (index_storage_variants[dof_access_cell]
                                  [cell_indices_face[v] / length] >=
            IndexStorageVariants::contiguous)
          dof_indices_interleave_strides[face_index][face * length + v] =
      ...
      if (is_interleaved || is_contiguous)
        for (unsigned int v = 0;
             v < n_vectorization_lanes_filled[face_index][face]; ++v)
      ...
      if (is_interleaved) {
        ...
        if (is_also_contiguous) {
          ...
          for (unsigned int v = 0;
               v < n_vectorization_lanes_filled[face_index][face]; ++v)
            if (dof_indices_interleave_strides[dof_access_cell]
                                              [cell_indices_face[v]] != length)
    ...
    if (faces[face].cells_exterior[0] != numbers::invalid_unsigned_int)
      face_computation(dof_access_face_exterior, faces[face].cells_exterior);
  }
}

template <int length>
void DoFInfo::compute_vector_zero_access_pattern(
  ...
  for (unsigned int part = 0; part < task_info.partition_row_index.size() - 2;
    ...
    for (unsigned int chunk = task_info.partition_row_index[part];
      ...
      if (faces.size() > 0) {
        for (unsigned int face = task_info.face_partition_data[chunk];
          ...
          for (unsigned int v = 0; v < vectorization_length; ++v) {
            if (faces[face].cells_interior[v] != numbers::invalid_unsigned_int)
              cells_in_interval.push_back(faces[face].cells_interior[v]);
            if (faces[face].cells_exterior[v] != numbers::invalid_unsigned_int)
              cells_in_interval.push_back(faces[face].cells_exterior[v]);
          }
        for (unsigned int face = task_info.boundary_partition_data[chunk];
             face < task_info.boundary_partition_data[chunk + 1]; ++face)
          for (unsigned int v = 0; v < vectorization_length; ++v)
            if (faces[face].cells_interior[v] != numbers::invalid_unsigned_int)
              cells_in_interval.push_back(faces[face].cells_interior[v]);
        ...
        if (task_info.refinement_edge_face_partition_data.empty() == false)
          for (unsigned int face =
                   task_info.refinement_edge_face_partition_data[chunk];
               face < task_info.refinement_edge_face_partition_data[chunk + 1];
               ++face)
            for (unsigned int v = 0; v < vectorization_length; ++v) {
      ...
      cells_in_interval.erase(
          std::unique(cells_in_interval.begin(), cells_in_interval.end()),
      ...
      for (const unsigned int cell : cells_in_interval) {
        for (unsigned int it = row_starts[cell * n_components].first;
          ...
          if (touched_first_by[myindex] == numbers::invalid_unsigned_int)
  ...
  convert_map_to_range_list(
      n_partitions, chunk_must_zero_vector, vector_zero_range_list_index,
      vector_zero_range_list, vector_partitioner->locally_owned_size());
  ...
  touched_first_by.resize(
      (vector_partitioner->locally_owned_size() + chunk_size_zero_vector - 1) /
  ...
  convert_map_to_range_list(n_partitions + 1, chunk_must_do_pre,
  ...
  convert_map_to_range_list(n_partitions + 1, chunk_must_do_post,
                            cell_loop_post_list_index, cell_loop_post_list,
                            vector_partitioner->locally_owned_size());
}
...
void DoFInfo::print_memory_consumption(StreamType &out,
...
template <typename Number>
void DoFInfo::print(const std::vector<Number> &constraint_pool_data,
  ...
  for (unsigned int row = 0; row < n_rows; ++row) {
    ...
    for (; con_it != end_con; ++con_it) {
      ...
      for (unsigned int k = constraint_pool_row_index[con_it->second];
           k < constraint_pool_row_index[con_it->second + 1]; k++, index++) {
        Assert(glob_indices + index != end_row, ExcInternalError());
