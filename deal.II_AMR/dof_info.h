...
template <int dim> class HangingNodes;
...
template <typename> class AffineConstraints;

class DynamicSparsityPattern;

template <typename> class TriaIterator;

template <int, int, bool> class DoFCellAccessor;
...
struct DoFInfo {
   ...
   * If @p with_constraints is `true`, then the returned vector will contain
   * indices required to resolve constraints.
   *
   ...
   * `std::vector::erase()`.
   */
  void get_dof_indices_on_cell_batch(std::vector<unsigned int> &local_indices,
  ...
  void read_dof_indices(
      ...
      ConstraintValues<double> &constraint_values, bool &cell_at_boundary);
   ...
   */
  template <int dim>
  bool process_hanging_node_constraints(
   ...
   */
  void compute_cell_index_compression(
      const std::vector<unsigned char> &irregular_cells);
   ...
   */
  template <int length>
  void compute_face_index_compression(
   ...
   */
  void make_connectivity_graph(const TaskInfo &task_info,
                               const std::vector<unsigned int> &renumbering,
   ...
   */
  void compute_tight_partitioners(
      const Table<2, ShapeInfo<double>> &shape_info,
      const unsigned int n_owned_cells, const unsigned int n_lanes,
      const std::vector<FaceToCellTopology<1>> &inner_faces,
      const std::vector<FaceToCellTopology<1>> &ghosted_faces,
      const std::vector<FaceToCellTopology<1>> &refinement_edge_faces,
      const bool fill_cell_centric, const MPI_Comm communicator_sm,
      const bool use_vector_data_exchanger_full);
   ...
   */
  void compute_shared_memory_contiguous_indices(
   ...
   */
  template <int length>
  void compute_vector_zero_access_pattern(
   ...
   */
  std::shared_ptr<const internal::MatrixFreeFunctions::VectorDataExchange::Base>
      vector_exchanger;
