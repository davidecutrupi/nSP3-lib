...
public:
   ...
   * @p mapping_update_flags_boundary_faces, @p
   * mapping_update_flags_inner_faces, and @p
   * mapping_update_flags_faces_by_cells specify the update flags that should be
   * stored by this class.
   *
   * The parameter @p mg_level specifies the level in the triangulation from
   * which the indices are to be used. If the level is set to
    ...
    AdditionalData(
        ...
        : tasks_parallel_scheme(tasks_parallel_scheme),
          ...
          mapping_update_flags_faces_by_cells(
              mapping_update_flags_faces_by_cells),
          mg_level(mg_level), store_plain_indices(store_plain_indices),
          ...
          store_ghost_cells(false), communicator_sm(MPI_COMM_SELF) {}
    ...
    AdditionalData(const AdditionalData &other)
        : tasks_parallel_scheme(other.tasks_parallel_scheme),
          ...
          mapping_update_flags_inner_faces(
              other.mapping_update_flags_inner_faces),
          mapping_update_flags_faces_by_cells(
  ...
  void reinit(const MappingType &mapping, const DoFHandler<dim> &dof_handler,
   ...
   */
  template <typename QuadratureType, typename number2, typename MappingType>
  void reinit(const MappingType &mapping,
  ...
  cell_loop(const std::function<void(
            ...
            OutVector &dst, const InVector &src,
  ...
  void cell_loop(void (CLASS::*cell_operation)(
                     const MatrixFree &, OutVector &, const InVector &,
                     const std::pair<unsigned int, unsigned int> &) const,
                 const CLASS *owning_class, OutVector &dst, const InVector &src,
                 const bool zero_dst_vector = false) const;
  ...
  void cell_loop(void (CLASS::*cell_operation)(
                 ...
                 CLASS *owning_class, OutVector &dst, const InVector &src,
                 const bool zero_dst_vector = false) const;
   ...
   * function object.
   *
   * @param inner_face_operation `std::function` with the signature
   * <tt>inner_face_operation (const MatrixFree<dim,Number> &, OutVector &,
   ...
   * `src` that will happen inside the body of the @p inner_face_operation
   * function, in analogy to `dst_vector_face_access`.
   */
  template <typename OutVector, typename InVector>
  void
  loop(const std::function<void(
           const MatrixFree<dim, Number, VectorizedArrayType> &, OutVector &,
           const InVector &, const std::pair<unsigned int, unsigned int> &)>
           &cell_operation,
       ...
       OutVector &dst, const InVector &src, const bool zero_dst_vector = false,
   ...
   * calls.
   *
   * @param boundary_face_operation Pointer to member function of `CLASS` with
   * the signature <tt>boundary_face_operation (const MatrixFree<dim,Number> &,
   * OutVector
   * &, InVector &, std::pair<unsigned int,unsigned int> &)</tt> in analogy to
   * `cell_operation` and `inner_face_operation`, but now the part
   * associated to the work on boundary faces. Boundary faces are separated by
   ...
   */
  template <typename CLASS, typename OutVector, typename InVector>
  void loop(void (CLASS::*cell_operation)(
                const MatrixFree &, OutVector &, const InVector &,
                const std::pair<unsigned int, unsigned int> &) const,
  ...
  loop_cell_centric(void (CLASS::*cell_operation)(
                        const MatrixFree &, OutVector &, const InVector &,
                        const std::pair<unsigned int, unsigned int> &) const,
                    const CLASS *owning_class, OutVector &dst,
                    const InVector &src, const bool zero_dst_vector = false,
                    const DataAccessOnFaces src_vector_face_access =
                        DataAccessOnFaces::unspecified) const;
   ...
   */
  template <typename CLASS, typename OutVector, typename InVector>
  void loop_cell_centric(void (CLASS::*cell_operation)(
                             const MatrixFree &, OutVector &, const InVector &,
                             const std::pair<unsigned int, unsigned int> &),
                         CLASS *owning_class, OutVector &dst,
                         const InVector &src,
                         const bool zero_dst_vector = false,
                         const DataAccessOnFaces src_vector_face_access =
                             DataAccessOnFaces::unspecified) const;
   ...
   */
  template <typename OutVector, typename InVector>
  void loop_cell_centric(
      const std::function<
          void(const MatrixFree &, OutVector &, const InVector &,
               const std::pair<unsigned int, unsigned int> &)> &cell_operation,
      OutVector &dst, const InVector &src, const bool zero_dst_vector = false,
      const DataAccessOnFaces src_vector_face_access =
          DataAccessOnFaces::unspecified) const;
   ...
   */
  std::pair<unsigned int, unsigned int> create_cell_subrange_hp_by_index(
   ...
   */
  unsigned int
  get_cell_active_fe_index(const std::pair<unsigned int, unsigned int> range,
   ...
   */
  unsigned int get_matrix_free_cell_index(
  ...
  get_face_iterator(const unsigned int face_batch_index,
                    const unsigned int lane_index, const bool interior = true,
   ...
   */
  unsigned int get_n_q_points(const unsigned int quad_index = 0,
                              const unsigned int hp_active_fe_index = 0) const;
   ...
   */
  unsigned int get_cell_range_category(
   ...
   * range of faces.
   */
  std::pair<unsigned int, unsigned int> get_face_range_category(
   ...
   */
  unsigned int get_cell_category(const unsigned int cell_batch_index,
                                 const unsigned int dof_handler_index =
                                     numbers::invalid_unsigned_int) const;
   ...
   */
  std::pair<unsigned int, unsigned int>
  get_face_category(const unsigned int face_batch_index,
                    const unsigned int dof_handler_index =
                        numbers::invalid_unsigned_int) const;
   ...
   */
  void release_scratch_data_non_threadsafe(
...
template <int dim, typename Number, typename VectorizedArrayType>
template <typename VectorType>
inline void MatrixFree<dim, Number, VectorizedArrayType>::initialize_dof_vector(
    VectorType &vec, const unsigned int comp) const {
...
template <int dim, typename Number, typename VectorizedArrayType>
inline unsigned int
MatrixFree<dim, Number, VectorizedArrayType>::n_ghost_inner_face_batches()
    const {
  if (task_info.face_partition_data.empty())
    return 0;
...
MatrixFree<dim, Number, VectorizedArrayType>::get_boundary_id(
  ...
  Assert(face_batch_index >= task_info.boundary_partition_data[0] &&
         ...
         ExcIndexRange(face_batch_index, task_info.boundary_partition_data[0],
...
MatrixFree<dim, Number, VectorizedArrayType>::get_faces_by_cells_boundary_id(
    const unsigned int cell_batch_index, const unsigned int face_number) const {
  ...
  for (unsigned int v = 0;
       v < n_active_entries_per_cell_batch(cell_batch_index); ++v)
    result[v] =
...
template <int dim, typename Number, typename VectorizedArrayType>
inline unsigned int
MatrixFree<dim, Number, VectorizedArrayType>::n_constraint_pool_entries()
    const {
...
MatrixFree<dim, Number, VectorizedArrayType>::create_cell_subrange_hp(
    const std::pair<unsigned int, unsigned int> &range,
    const unsigned int degree, const unsigned int dof_handler_component) const {
  if (dof_info[dof_handler_component].cell_active_fe_index.empty()) {
    AssertDimension(dof_info[dof_handler_component].fe_index_conversion.size(),
                    1);
    AssertDimension(
  ...
  if (fe_index >= dof_info[dof_handler_component].max_fe_index)
    ...
    return create_cell_subrange_hp_by_index(range, fe_index,
                                            dof_handler_component);
}

template <int dim, typename Number, typename VectorizedArrayType>
inline bool MatrixFree<dim, Number, VectorizedArrayType>::at_irregular_cell(
  ...
  return VectorizedArrayType::size() > 1 &&
         cell_level_index[(cell_batch_index + 1) * VectorizedArrayType::size() -
                          1] ==
             cell_level_index[(cell_batch_index + 1) *
                                  VectorizedArrayType::size() -
                              2];
}
...
MatrixFree<dim, Number, VectorizedArrayType>::get_face_active_fe_index(
    const std::pair<unsigned int, unsigned int> range,
    const bool is_interior_face, const unsigned int dof_handler_index) const {
  const unsigned int dof_handler_index_local =
      dof_handler_index == numbers::invalid_unsigned_int
          ? first_hp_dof_handler_index
          : dof_handler_index;
  ...
  if (is_interior_face) {
    ...
    for (unsigned int i = range.first; i < range.second; ++i)
      AssertDimension(index, fe_indices[face_info.faces[i].cells_interior[0] /
                                        VectorizedArrayType::size()]);
    ...
    for (unsigned int i = range.first; i < range.second; ++i)
      AssertDimension(index, fe_indices[face_info.faces[i].cells_exterior[0] /
                                        VectorizedArrayType::size()]);
...
MatrixFree<dim, Number, VectorizedArrayType>::get_n_q_points(
    const unsigned int quad_index, const unsigned int active_fe_index) const {
...
MatrixFree<dim, Number, VectorizedArrayType>::get_shape_info(
    const unsigned int dof_handler_index, const unsigned int index_quad,
    const unsigned int index_fe, const unsigned int active_fe_index,
...
MatrixFree<dim, Number, VectorizedArrayType>::get_face_category(
  ...
  for (unsigned int v = 0;
    ...
    result.first = std::max(
        result.first, dof_info[dof_handler_index_local].cell_active_fe_index
                          [face_info.faces[face_batch_index].cells_interior[v] /
                           VectorizedArrayType::size()]);
...
template <int dim, typename Number, typename VectorizedArrayType>
void MatrixFree<dim, Number, VectorizedArrayType>::release_scratch_data(
...
template <int dim, typename Number, typename VectorizedArrayType>
void MatrixFree<dim, Number, VectorizedArrayType>::
...
inline std::vector<IndexSet> extract_locally_owned_index_sets(
...
template <int dim, typename Number, typename VectorizedArrayType>
template <typename QuadratureType, typename number2, typename MappingType>
void MatrixFree<dim, Number, VectorizedArrayType>::reinit(
    const MappingType &mapping, const DoFHandler<dim> &dof_handler,
  ...
  internal_reinit(std::make_shared<hp::MappingCollection<dim>>(mapping),
                  dof_handlers, constraints, locally_owned_sets, quad_hp,
                  additional_data);
}
...
void MatrixFree<dim, Number, VectorizedArrayType>::reinit(
  ...
  internal_reinit(std::make_shared<hp::MappingCollection<dim>>(mapping),
                  dof_handler, constraint, locally_owned_set, quad_hp,
                  additional_data);
}
...
struct VectorDataExchange {
  ...
  VectorDataExchange(
      ...
      const typename dealii::MatrixFree<dim, Number,
      ...
      : matrix_free(matrix_free),
        ...
        ,
        tmp_data(n_components), requests(n_components)
#endif
  {
    (void)n_components;
    if (this->vector_face_access !=
        dealii::MatrixFree<dim, Number,
                           VectorizedArrayType>::DataAccessOnFaces::unspecified)
  ...
  void compress_start(const unsigned int component_in_block_vector,
    ...
    if (vec.size() != 0) {
      ...
      part.import_from_ghosted_array_start(
          VectorOperation::add, component_in_block_vector * 2 + channel_shift,
  ...
  template <typename VectorType,
            std::enable_if_t<is_not_parallel_vector<VectorType>, VectorType> * =
                nullptr>
  void reset_ghost_values(const VectorType & /*vec*/) const {}
    ...
    if (vec.size() != 0) {
      ...
      if (part.n_ghost_indices() > 0) {
        part.reset_ghost_values(ArrayView<Number>(
            const_cast<VectorType &>(vec).begin() + part.locally_owned_size(),
            matrix_free.get_dof_info(mf_component)
                .vector_partitioner->n_ghost_indices()));
      }
  ...
  void zero_vector_region(const unsigned int range_index,
    ...
    if (range_index == numbers::invalid_unsigned_int)
      ...
      for (unsigned int id = dof_info.vector_zero_range_list_index[range_index];
        ...
        std::memset(vec.begin() + dof_info.vector_zero_range_list[id].first, 0,
  ...
  void zero_vector_region(const unsigned int range_index,
    ...
    if (range_index == numbers::invalid_unsigned_int || range_index == 0) {
      if constexpr (std::is_same_v<ArrayView<typename VectorType::value_type>,
                                   VectorType>) {
   ...
   */
  void zero_vector_region(const unsigned int, ...) const {
  ...
  const dealii::MatrixFree<dim, Number, VectorizedArrayType> &matrix_free;
  const typename dealii::MatrixFree<
      dim, Number, VectorizedArrayType>::DataAccessOnFaces vector_face_access;
...
template <typename VectorStruct>
inline unsigned int n_components(const std::vector<VectorStruct> &vec) {
  unsigned int components = 0;
...
template <typename VectorType,
          std::enable_if_t<is_not_parallel_vector<VectorType>, VectorType> * =
              nullptr>
bool has_ghost_elements(const VectorType &vec) {
  (void)vec;
  return false;
}
...
void update_ghost_values_start(
  ...
  if (get_communication_block_size(vec) < vec.n_blocks()) {
    ...
    Assert(
        exchanger.matrix_free.get_task_info().allow_ghosted_vectors_in_loops ||
            ghosts_set == false,
        ExcNotImplemented());
...
template <int dim, typename VectorStruct, typename Number,
          ...
          std::enable_if_t<IsBlockVector<VectorStruct>::value, VectorStruct> * =
              nullptr>
void update_ghost_values_finish(
    const VectorStruct &vec,
    VectorDataExchange<dim, Number, VectorizedArrayType> &exchanger,
    const unsigned int channel = 0) {
  if (get_communication_block_size(vec) < vec.n_blocks()) {
...
template <int dim, typename VectorStruct, typename Number,
          typename VectorizedArrayType>
inline void update_ghost_values_finish(
    const std::vector<VectorStruct> &vec,
    VectorDataExchange<dim, Number, VectorizedArrayType> &exchanger) {
  unsigned int component_index = 0;
  for (unsigned int comp = 0; comp < vec.size(); ++comp) {
...
template <int dim, typename VectorStruct, typename Number,
          ...
          std::enable_if_t<IsBlockVector<VectorStruct>::value, VectorStruct> * =
              nullptr>
inline void reset_ghost_values(
    const VectorStruct &vec,
    VectorDataExchange<dim, Number, VectorizedArrayType> &exchanger) {
...
template <int dim, typename VectorStruct, typename Number,
          ...
          std::enable_if_t<IsBlockVector<VectorStruct>::value, VectorStruct> * =
              nullptr>
inline void zero_vector_region(
    const unsigned int range_index, VectorStruct &vec,
    VectorDataExchange<dim, Number, VectorizedArrayType> &exchanger) {
  for (unsigned int i = 0; i < vec.n_blocks(); ++i)
...
template <int dim, typename VectorStruct, typename Number,
          typename VectorizedArrayType>
inline void zero_vector_region(
    const unsigned int range_index, std::vector<VectorStruct> &vec,
    VectorDataExchange<dim, Number, VectorizedArrayType> &exchanger) {
  for (unsigned int comp = 0; comp < vec.size(); ++comp)
...
apply_operation_to_constrained_dofs(const std::vector<unsigned int> &,
                                    const VectorStruct1 &, VectorStruct2 &) {}

template <typename Number>
inline void apply_operation_to_constrained_dofs(
...
struct InterfaceSelector<MF, InVector, OutVector, Container, true> {
  using function_type =
      void (Container::*)(const MF &, OutVector &, const InVector &,
                          const std::pair<unsigned int, unsigned int> &) const;
};
...
public:
  ...
  using function_type = typename MatrixFreeFunctions::InterfaceSelector<
      MF, InVector, OutVector, Container, is_constant>::function_type;

  // constructor, binds all the arguments to this class
  MFWorker(const MF &matrix_free, const InVector &src, OutVector &dst,
           const bool zero_dst_vector_setting, const Container &container,
           function_type cell_function, function_type face_function,
           ...
           const std::function<void(const unsigned int, const unsigned int)>
               &operation_after_loop = {},
           const unsigned int dof_handler_index_pre_post = 0)
      : matrix_free(matrix_free), container(const_cast<Container &>(container)),
        cell_function(cell_function), face_function(face_function),
        boundary_function(boundary_function), src(src), dst(dst),
  ...
  cell(const std::pair<unsigned int, unsigned int> &cell_range) override {
    if (cell_function != nullptr && cell_range.second > cell_range.first)
      for (unsigned int i = 0; i < matrix_free.n_active_fe_indices(); ++i) {
        ...
        (container.*cell_function)(matrix_free, this->dst, this->src,
                                   cell_subrange);
      }
  }

  virtual void cell(const unsigned int range_index) override {
    process_range(
        cell_function, matrix_free.get_task_info().cell_partition_data_hp_ptr,
        matrix_free.get_task_info().cell_partition_data_hp, range_index);
  }

  virtual void face(const unsigned int range_index) override {
    process_range(
        face_function, matrix_free.get_task_info().face_partition_data_hp_ptr,
        matrix_free.get_task_info().face_partition_data_hp, range_index);
  }

  virtual void boundary(const unsigned int range_index) override {
  ...
  virtual void refinement_edge_face(const unsigned int range_index) override {
    process_range(
        face_function,
        matrix_free.get_task_info().refinement_edge_face_partition_data_hp_ptr,
        matrix_free.get_task_info().refinement_edge_face_partition_data_hp,
        range_index);
  }

private:
  void process_range(const function_type &fu,
    ...
    for (unsigned int i = ptr[range_index]; i < ptr[range_index + 1]; ++i) {
      ...
      (container.*fu)(matrix_free, this->dst, this->src,
...
public:
  ...
  virtual void cell_loop_pre_range(const unsigned int range_index) override {
    if (operation_before_loop) {
      ...
      if (range_index == numbers::invalid_unsigned_int) {
        ...
        dealii::parallel::apply_to_subranges(
            0U, dof_info.vector_partitioner->locally_owned_size(),
            operation_before_loop,
        ...
        for (unsigned int id = dof_info.cell_loop_pre_list_index[range_index];
             id != dof_info.cell_loop_pre_list_index[range_index + 1]; ++id)
  ...
  virtual void cell_loop_post_range(const unsigned int range_index) override {
    if (operation_after_loop) {
      ...
      if (range_index ==
        ...
        apply_operation_to_constrained_dofs(
            matrix_free.get_constrained_dofs(dof_handler_index_pre_post), src,
            dst);
      ...
      if (range_index == numbers::invalid_unsigned_int) {
        ...
        for (unsigned int id = dof_info.cell_loop_post_list_index[range_index];
             id != dof_info.cell_loop_post_list_index[range_index + 1]; ++id)
...
struct MFClassWrapper {
  using function_type =
      std::function<void(const MF &, OutVector &, const InVector &,
                         const std::pair<unsigned int, unsigned int> &)>;

  MFClassWrapper(const function_type cell, const function_type face,
                 const function_type boundary)
      : cell(cell), face(face), boundary(boundary) {}
  ...
  void boundary_integrator(
      const MF &mf, OutVector &dst, const InVector &src,
      const std::pair<unsigned int, unsigned int> &range) const {
    if (boundary)
...
template <int dim, typename Number, typename VectorizedArrayType>
template <typename OutVector, typename InVector>
inline void MatrixFree<dim, Number, VectorizedArrayType>::cell_loop(
    const std::function<
        void(const MatrixFree<dim, Number, VectorizedArrayType> &, OutVector &,
             const InVector &, const std::pair<unsigned int, unsigned int> &)>
        &cell_operation,
    OutVector &dst, const InVector &src, const bool zero_dst_vector) const {
  using Wrapper =
      internal::MFClassWrapper<MatrixFree<dim, Number, VectorizedArrayType>,
                               InVector, OutVector>;
  Wrapper wrap(cell_operation, nullptr, nullptr);
  internal::MFWorker<MatrixFree<dim, Number, VectorizedArrayType>, InVector,
...
inline void MatrixFree<dim, Number, VectorizedArrayType>::cell_loop(
  ...
  internal::MFWorker<MatrixFree<dim, Number, VectorizedArrayType>, InVector,
      ...
      worker(*this, src, dst, false, wrap, &Wrapper::cell_integrator,
             &Wrapper::face_integrator, &Wrapper::boundary_integrator,
             DataAccessOnFaces::none, DataAccessOnFaces::none,
             operation_before_loop, operation_after_loop,
...
template <int dim, typename Number, typename VectorizedArrayType>
template <typename OutVector, typename InVector>
inline void MatrixFree<dim, Number, VectorizedArrayType>::loop(
    const std::function<
        void(const MatrixFree<dim, Number, VectorizedArrayType> &, OutVector &,
             const InVector &, const std::pair<unsigned int, unsigned int> &)>
        &cell_operation,
    ...
    const std::function<
        void(const MatrixFree<dim, Number, VectorizedArrayType> &, OutVector &,
             const InVector &, const std::pair<unsigned int, unsigned int> &)>
        &boundary_face_operation,
    OutVector &dst, const InVector &src, const bool zero_dst_vector,
    const DataAccessOnFaces dst_vector_face_access,
...
inline void MatrixFree<dim, Number, VectorizedArrayType>::cell_loop(
    void (CLASS::*function_pointer)(
        const MatrixFree<dim, Number, VectorizedArrayType> &, OutVector &,
        const InVector &, const std::pair<unsigned int, unsigned int> &) const,
    const CLASS *owning_class, OutVector &dst, const InVector &src,
    const bool zero_dst_vector) const {
  internal::MFWorker<MatrixFree<dim, Number, VectorizedArrayType>, InVector,
                     OutVector, CLASS, true>
      worker(*this, src, dst, zero_dst_vector, *owning_class, function_pointer,
             nullptr, nullptr);
  task_info.loop(worker);
}
...
inline void MatrixFree<dim, Number, VectorizedArrayType>::cell_loop(
  ...
  internal::MFWorker<MatrixFree<dim, Number, VectorizedArrayType>, InVector,
                     OutVector, CLASS, true>
      worker(*this, src, dst, false, *owning_class, function_pointer, nullptr,
             nullptr, DataAccessOnFaces::none, DataAccessOnFaces::none,
             operation_before_loop, operation_after_loop,
             dof_handler_index_pre_post);
  task_info.loop(worker);
}
...
inline void MatrixFree<dim, Number, VectorizedArrayType>::loop(
    ...
    void (CLASS::*boundary_face_operation)(
        ...
        const InVector &, const std::pair<unsigned int, unsigned int> &) const,
    const CLASS *owning_class, OutVector &dst, const InVector &src,
    const bool zero_dst_vector, const DataAccessOnFaces dst_vector_face_access,
    const DataAccessOnFaces src_vector_face_access) const {
  internal::MFWorker<MatrixFree<dim, Number, VectorizedArrayType>, InVector,
                     OutVector, CLASS, true>
      worker(*this, src, dst, zero_dst_vector, *owning_class, cell_operation,
             inner_face_operation, boundary_face_operation,
             src_vector_face_access, dst_vector_face_access);
  task_info.loop(worker);
}
...
template <typename CLASS, typename OutVector, typename InVector>
inline void MatrixFree<dim, Number, VectorizedArrayType>::loop(
    void (CLASS::*cell_operation)(const MatrixFree &, OutVector &,
                                  const InVector &,
                                  const std::pair<unsigned int, unsigned int> &)
        const,
    void (CLASS::*inner_face_operation)(
  ...
  internal::MFWorker<MatrixFree<dim, Number, VectorizedArrayType>, InVector,
                     OutVector, CLASS, true>
      worker(*this, src, dst, false, *owning_class, cell_operation,
             inner_face_operation, boundary_face_operation,
             src_vector_face_access, dst_vector_face_access,
             operation_before_loop, operation_after_loop,
             dof_handler_index_pre_post);
...
template <int dim, typename Number, typename VectorizedArrayType>
template <typename CLASS, typename OutVector, typename InVector>
inline void MatrixFree<dim, Number, VectorizedArrayType>::loop_cell_centric(
    void (CLASS::*function_pointer)(
        const MatrixFree<dim, Number, VectorizedArrayType> &, OutVector &,
        const InVector &, const std::pair<unsigned int, unsigned int> &) const,
    const CLASS *owning_class, OutVector &dst, const InVector &src,
  ...
  internal::MFWorker<MatrixFree<dim, Number, VectorizedArrayType>, InVector,
                     OutVector, CLASS, true>
      worker(*this, src, dst, zero_dst_vector, *owning_class, function_pointer,
             nullptr, nullptr, src_vector_face_access_temp,
...
inline void MatrixFree<dim, Number, VectorizedArrayType>::loop_cell_centric(
  ...
  internal::MFWorker<MatrixFree<dim, Number, VectorizedArrayType>, InVector,
      ...
      worker(*this, src, dst, zero_dst_vector, wrap, &Wrapper::cell_integrator,
             &Wrapper::face_integrator, &Wrapper::boundary_integrator,
             src_vector_face_access_temp, DataAccessOnFaces::none);
  task_info.loop(worker);
}
