...
MatrixFree<dim, Number, VectorizedArrayType>::MatrixFree()
    : EnableObserverPointer(), indices_are_initialized(false),
      mapping_is_initialized(false), mg_level(numbers::invalid_unsigned_int),
...
MatrixFree<dim, Number, VectorizedArrayType>::create_cell_subrange_hp_by_index(
    ...
    const unsigned int fe_index, const unsigned int dof_handler_index) const {
  ...
  if (fe_indices.empty() == true ||
    ...
    if constexpr (running_in_debug_mode()) {
      for (unsigned int i = range.first + 1; i < range.second; ++i)
        Assert(fe_indices[i] >= fe_indices[i - 1],
               ExcMessage("Cell range must be over sorted range of FE indices "
                          "in hp-case!"));
    ...
    return_range.first =
        std::lower_bound(fe_indices.begin() + range.first,
                         fe_indices.begin() + range.second, fe_index) -
        fe_indices.begin();
    return_range.second =
        std::lower_bound(fe_indices.begin() + return_range.first,
                         fe_indices.begin() + range.second, fe_index + 1) -
        fe_indices.begin();
...
public:
  FaceRangeComparator(const std::vector<unsigned int> &fe_indices,
                      const bool include, const bool only_face_type)
      : fe_indices(fe_indices), include(include),
        only_face_type(only_face_type) {}

  template <int vectorization_width>
  bool operator()(const internal::MatrixFreeFunctions::FaceToCellTopology<
...
template <int dim, typename Number, typename VectorizedArrayType>
void MatrixFree<dim, Number, VectorizedArrayType>::renumber_dofs(
...
MatrixFree<dim, Number, VectorizedArrayType>::get_affine_constraints(
  ...
  AssertThrow(
      affine_constraints[dof_handler_index] != nullptr,
      ExcMessage("AffineConstraints are only accessible if its Number type "
...
MatrixFree<dim, Number, VectorizedArrayType>::get_cell_iterator(
    const unsigned int cell_batch_index, const unsigned int lane_index,
    const unsigned int dof_handler_index) const {
  ...
  return typename DoFHandler<dim>::cell_iterator(
      &dof_handlers[dof_handler_index]->get_triangulation(), index.first,
      index.second, &*dof_handlers[dof_handler_index]);
}

template <int dim, typename Number, typename VectorizedArrayType>
std::pair<int, int>
MatrixFree<dim, Number, VectorizedArrayType>::get_cell_level_and_index(
    const unsigned int cell_batch_index, const unsigned int lane_index) const {
  AssertIndexRange(cell_batch_index, task_info.cell_partition_data.back());
  AssertIndexRange(lane_index,
                   n_active_entries_per_cell_batch(cell_batch_index));
...
MatrixFree<dim, Number, VectorizedArrayType>::get_matrix_free_cell_index(
  ...
  return mf_cell_indices[(this->get_mg_level() == numbers::invalid_unsigned_int)
...
MatrixFree<dim, Number, VectorizedArrayType>::get_face_iterator(
    const unsigned int face_batch_index, const unsigned int lane_index,
    const bool interior, const unsigned int fe_component) const {
  ...
  const unsigned int cell_index =
      interior ? face2cell_info.cells_interior[lane_index]
  ...
  return {typename DoFHandler<dim>::cell_iterator(
              &dof_handlers[fe_component]->get_triangulation(), index.first,
              index.second, &*dof_handlers[fe_component]),
          interior ? face2cell_info.interior_face_no
                   : face2cell_info.exterior_face_no};
}

template <int dim, typename Number, typename VectorizedArrayType>
void MatrixFree<dim, Number, VectorizedArrayType>::copy_from(
...
void MatrixFree<dim, Number, VectorizedArrayType>::internal_reinit(
  ...
  // gradients and Hessians for quadrature points.
  {
    ...
    for (unsigned int no = 0; no < dof_handler.size(); ++no)
      n_fe_in_collection = std::max(
          n_fe_in_collection, dof_handler[no]->get_fe_collection().size());
    ...
    shape_info.reinit(TableIndices<4>(n_components, n_quad, n_fe_in_collection,
    ...
    for (unsigned int no = 0, c = 0; no < dof_handler.size(); ++no)
      for (unsigned int b = 0; b < dof_handler[no]->get_fe(0).n_base_elements();
        ...
        for (unsigned int fe_no = 0;
             fe_no < dof_handler[no]->get_fe_collection().size(); ++fe_no)
  ...
  if (additional_data.initialize_indices == true) {
    ...
    if constexpr (running_in_debug_mode()) {
      for (const auto &constraint : constraints)
        Assert(constraint->is_closed(task_info.communicator),
               ExcMessage(
                   "You have provided a non-empty AffineConstraints object "
                   "that has not "
  ...
  // initialize bare structures
    ...
    task_info.create_blocks_serial(dummy, 1, false, dummy, false, dummy, dummy,
                                   dummy2);
  ...
  if (task_info.scheme != internal::MatrixFreeFunctions::TaskInfo::
        ...
        if (task_info.cell_partition_data[i + 1] >
          ...
          if (range.second > range.first)
            for (unsigned int i = 0; i < this->n_active_fe_indices(); ++i) {
              const auto cell_subrange = this->create_cell_subrange_hp_by_index(
    ...
    if (task_info.face_partition_data.empty() == false) {
        ...
        if (fe_indices.empty() == true && n_face_types == 1)
          ...
          return_range.first =
              std::lower_bound(
                  ...
                  internal::FaceRangeComparator(fe_indices, false,
                                                only_face_type)) -
          ...
          return_range.second =
              std::lower_bound(
                  ...
                  std::array<unsigned int, 3>{
                      {face_type, fe_index_interior, fe_index_exterior}},
                  internal::FaceRangeComparator(fe_indices, true,
                                                only_face_type)) -
              face_info.faces.begin();
          Assert(return_range.first >= range.first &&
                     return_range.second <= range.second,
      ...
      for (unsigned int i = 0; i < task_info.face_partition_data.size() - 1;
        ...
        if (task_info.face_partition_data[i + 1] >
          ...
          if (range.second > range.first)
            for (unsigned int t = 0; t < std::max<unsigned int>(dim - 1, 1);
                 ++t)
              for (unsigned int i = 0; i < this->n_active_fe_indices(); ++i)
                for (unsigned int j = 0; j < this->n_active_fe_indices(); ++j) {
                  const auto subrange = create_inner_face_subrange_hp_by_index(
    ...
    if (task_info.boundary_partition_data.empty() == false) {
      ...
      const auto create_boundary_face_subrange_hp_by_index =
          [&](const std::pair<unsigned int, unsigned int> &range,
              const unsigned int face_type, const unsigned int fe_index,
              const unsigned int dof_handler_index)
          -> std::pair<unsigned int, unsigned int> {
        const unsigned int n_face_types = std::max<unsigned int>(dim - 1, 1);
    ...
    // ... for refinement edge faces
    if (task_info.refinement_edge_face_partition_data.empty() == false) {
      auto &ptr = task_info.refinement_edge_face_partition_data_hp_ptr;
      auto &data = task_info.refinement_edge_face_partition_data_hp;

      ptr = {0};
      data = {};
      data.reserve(2 * task_info.refinement_edge_face_partition_data.size());

      const auto create_refinement_face_subrange_hp_by_index =
          [&](const std::pair<unsigned int, unsigned int> &range,
              const unsigned int face_type,
              const unsigned int fe_index_interior,
              const unsigned int fe_index_exterior,
      ...
      for (unsigned int i = 0;
           i < task_info.refinement_edge_face_partition_data.size() - 1; ++i) {
        if (task_info.refinement_edge_face_partition_data[i + 1] >
            task_info.refinement_edge_face_partition_data[i]) {
          std::pair<unsigned int, unsigned int> range{
              task_info.refinement_edge_face_partition_data[i],
              task_info.refinement_edge_face_partition_data[i + 1]};
          ...
          if (range.second > range.first)
            for (unsigned int t = 0; t < std::max<unsigned int>(dim - 1, 1);
              ...
              for (unsigned int i = 0; i < this->n_active_fe_indices(); ++i)
                for (unsigned int j = 0; j < this->n_active_fe_indices(); ++j) {
                  const auto subrange =
                      create_refinement_face_subrange_hp_by_index(
                          range, t, i, j, first_hp_dof_handler_index);
  ...
  if (additional_data.initialize_mapping == true) {
    ...
    mapping_info.initialize(
        dof_handler[0]->get_triangulation(), cell_level_index, face_info,
        ...
        additional_data.mapping_update_flags_faces_by_cells, piola_transform);
  ...
  // set up map: deal.II index -> MatrixFree index
  {
    ...
    mf_cell_indices.resize(
        (mg_level == numbers::invalid_unsigned_int)
            ? tria.n_active_cells()
            : (mg_level < tria.n_levels() ? tria.n_raw_cells(mg_level) : 0),
        numbers::invalid_unsigned_int);

    for (unsigned int cell = 0;
         cell < n_cell_batches() + n_ghost_cell_batches(); ++cell)
...
template <int dim, typename Number, typename VectorizedArrayType>
void MatrixFree<dim, Number, VectorizedArrayType>::update_mapping(
...
void MatrixFree<dim, Number, VectorizedArrayType>::update_mapping(
  ...
  mapping_info.update_mapping(
      dof_handlers[0]->get_triangulation(), cell_level_index, face_info,
      dof_info[first_hp_dof_handler_index].cell_active_fe_index, mapping);
}

template <int dim, typename Number, typename VectorizedArrayType>
template <int spacedim>
bool MatrixFree<dim, Number, VectorizedArrayType>::is_supported(
...
template <int dim, typename Number, typename VectorizedArrayType>
void MatrixFree<dim, Number, VectorizedArrayType>::initialize_dof_handlers(
  ...
  if (additional_data.store_ghost_cells == true)
    cell_level_index.insert(cell_level_index.end(), ghosted_cell_index.begin(),
...
inline void fill_index_subrange(
  ...
  for (unsigned int cell = begin; cell < end; ++cell) {
    ...
    for (auto f : dcell->face_indices()) {
      ...
      if (dcell->at_boundary(f) == false &&
        ...
        auto neighbor = dcell->neighbor_or_periodic_neighbor(f);
        std::pair<unsigned int, unsigned int> level_index(neighbor->level(),
                                                          neighbor->index());
        auto it = map.find(level_index);
        if (it != map.end()) {
          const unsigned int neighbor_cell = it->second;
          if (neighbor_cell != cell && neighbor_cell < locally_owned_size)
        ...
        if (neighbor->has_children()) {
          ...
          for (unsigned int c = 0; c < n_children; ++c) {
            ...
            std::pair<unsigned int, unsigned int> child_level_index(
                child->level(), child->index());
            auto it_child = map.find(child_level_index);
            if (it_child != map.end()) {
              const unsigned int neighbor_cell = it_child->second;
              if (neighbor_cell != cell && neighbor_cell < locally_owned_size)
                new_indices.push_back(neighbor_cell);
            }
          }
    ...
    connectivity_direct.add_entries(
        cell, new_indices.begin(),
        std::unique(new_indices.begin(), new_indices.end()), true);
  }
}

inline void fill_connectivity_indirect_subrange(
    const unsigned int begin, const unsigned int end,
...
std::vector<bool> compute_dof_info(
    ...
    const unsigned int cell_level_index_end_local, const unsigned int mg_level,
    ...
    const bool do_face_integrals, const bool build_inner_faces,
  ...
  if (do_face_integrals)
    face_setup.initialize(dof_handlers[0]->get_triangulation(), mg_level,
                          hold_all_faces_to_owned_cells, build_inner_faces,
                          cell_level_index);
  ...
  if (use_fast_hanging_node_algorithm) {
    ...
    use_fast_hanging_node_algorithm = std::all_of(
        reference_cells.begin(), reference_cells.end(),
        [](const auto &r) { return r.is_hyper_cube() || r.is_simplex(); });
  }
    ...
    for (unsigned int fe_index = 0; fe_index < fes.size(); ++fe_index) {
      ...
      for (unsigned int c = 0; c < dof_info[no].n_base_elements; ++c) {
        dof_info[no].n_components[c] =
            fe.element_multiplicity(c) * fe.base_element(c).n_components();
        for (unsigned int l = 0; l < dof_info[no].n_components[c]; ++l) {
          ...
          dof_info[no].component_dof_indices_offset[fe_index].push_back(
              ...
              shape_infos(dof_info[no].global_base_element_offset + c, fe_index)
        ...
        lexicographic[no][fe_index].insert(lexicographic[no][fe_index].end(),
                                           lex.begin(), lex.end());
      }
    ...
    for (unsigned int no = 0; no < n_dof_handlers; ++no) {
      ...
      if ([](const auto &supported_components) {
            return std::none_of(supported_components.begin(),
                                supported_components.end(), [](const auto &a) {
    ...
    for (unsigned int no = 0; no < n_dof_handlers; ++no) {
      ...
      if (mg_level == numbers::invalid_unsigned_int) {
        typename DoFHandler<dim>::active_cell_iterator cell_it(
            &tria, cell_level_index[counter].first,
            cell_level_index[counter].second, &dofh);
        const unsigned int fe_index = dofh.get_fe_collection().size() > 1
                                          ? cell_it->active_fe_index()
                                          : 0;
        const unsigned int dofs_per_cell = dof_info[no].dofs_per_cell[fe_index];
        ...
        if (dim > 1 && use_fast_hanging_node_algorithm) {
          ...
          cell_has_hanging_node_constraints =
              dof_info[no].process_hanging_node_constraints(
                  *hanging_nodes, lexicographic[no], counter, cell_it,
      ...
      // we are requested to use a multigrid level
        ...
        typename DoFHandler<dim>::cell_iterator cell_it(
            &tria, cell_level_index[counter].first,
            cell_level_index[counter].second, &dofh);
        const unsigned int dofs_per_cell = dof_info[no].dofs_per_cell[0];
        local_dof_indices_resolved.resize(dofs_per_cell);
        ...
        if (cell_categorization_enabled) {
          AssertIndexRange(cell_it->index(),
                           cell_vectorization_category.size());
          dof_info[no].cell_active_fe_index[counter] =
              cell_vectorization_category[cell_level_index[counter].second];
        }
      ...
      dof_info[no].read_dof_indices(
          cell_has_hanging_node_constraints ? local_dof_indices_resolved
                                            : local_dof_indices,
          local_dof_indices, cell_has_hanging_node_constraints, *constraint[no],
          counter, constraint_values, cell_at_subdomain_boundary);
    }
            ...
            if (cell->level_subdomain_id() == task_info.my_pid)
              for (const unsigned int f : cell->face_indices())
                if ((cell->at_boundary(f) == false ||
                  ...
                  dof_indices.resize(cell->neighbor_or_periodic_neighbor(f)
                                         ->get_fe()
                                         .n_dofs_per_cell());
                  cell->neighbor_or_periodic_neighbor(f)->get_mg_dof_indices(
                      dof_indices);
                  for (const auto dof_index : dof_indices)
                    dof_info[no].ghost_dofs.push_back(dof_index);
                }
      }
      dof_info[no].assign_ghosts(cells_with_ghosts, task_info.communicator_sm,
  ...
  Assert(
      ...
      ExcMessage(
          ...
          "MatrixFree::Additional_data.tasks_parallel_scheme = MatrixFree<dim, "
          "double>::AdditionalData::none."));
  ...
  if (task_info.scheme == internal::MatrixFreeFunctions::TaskInfo::none) {
    ...
    const unsigned int n_levels = mg_level == numbers::invalid_unsigned_int
                                      ? tria.n_levels() - 1
                                      : mg_level;
    std::vector<std::vector<std::pair<
        unsigned int, std::array<unsigned int, max_children_per_cell>>>>
    ...
    task_info.create_blocks_serial(
        subdomain_boundary_cells, max_dofs_per_cell, hp_functionality_enabled,
        dof_info[first_hp_dof_handler_index].cell_active_fe_index,
        strict_categories, parent_relation, renumbering, irregular_cells);
  } else {
    ...
    if (task_info.scheme ==
      ...
      if (hp_functionality_enabled) {
        ...
        for (counter = 0; counter < std::min(start_nonboundary * n_lanes,
          ...
          renumbering_fe_index[dof_info_hp
        ...
        *(task_info.cell_partition_data.end() - 2) = n_cell_batches_before;
    ...
    if (do_face_integrals) {
      ...
      DynamicSparsityPattern connectivity_direct(connectivity.n_rows(),
                                                 connectivity.n_cols());
      dealii::parallel::apply_to_subranges(
          0, task_info.n_active_cells,
          [&cell_level_index, &tria, &map, &connectivity_direct](
              const unsigned int begin, const unsigned int end) {
            fill_connectivity_subrange<dim>(begin, end, tria, cell_level_index,
                                            map, connectivity_direct);
          },
          20);
      ...
      dealii::parallel::apply_to_subranges(
          ...
          [&connectivity_direct, &connectivity](const unsigned int begin,
                                                const unsigned int end) {
            fill_connectivity_indirect_subrange(begin, end, connectivity_direct,
                                                connectivity);
          },
          20);
#endif
    ...
    if (task_info.n_active_cells > 0)
      dof_info_hp.make_connectivity_graph(task_info, renumbering, connectivity);

    task_info.make_thread_graph(dof_info_hp.cell_active_fe_index, connectivity,
  ...
  for (unsigned int no = 0; no < n_dof_handlers; ++no)
    dof_info[no].reorder_cells(task_info, renumbering,
                               constraint_pool_row_index, irregular_cells);
...
void MatrixFree<dim, Number, VectorizedArrayType>::initialize_indices(
      ...
      shape_info.size(0), shape_info.size(2));
  {
    ...
    for (unsigned int no = 0, c = 0; no < dof_handlers.size(); ++no)
      for (unsigned int b = 0;
           b < dof_handlers[no]->get_fe(0).n_base_elements(); ++b, ++c)
        for (unsigned int fe_no = 0;
             fe_no < dof_handlers[no]->get_fe_collection().size(); ++fe_no)
          shape_info_dummy(c, fe_no).reinit(
              dof_handlers[no]->get_fe(fe_no).reference_cell() ==
                      ReferenceCells::get_hypercube<dim>()
                  ? quad
                  : quad_simplex,
              dof_handlers[no]->get_fe(fe_no), b);
  }
  ...
  const std::vector<bool> is_fe_dg = internal::compute_dof_info(
      constraint, locally_owned_dofs, dof_handlers, shape_info_dummy,
      cell_level_index_end_local, additional_data.mg_level,
      additional_data.hold_all_faces_to_owned_cells,
      additional_data.cell_vectorization_category,
      additional_data.cell_vectorization_categories_strict, do_face_integrals,
      additional_data.mapping_update_flags_inner_faces != update_default,
      overlap_communication_computation, task_info, cell_level_index, dof_info,
      face_setup, constraint_values,
  ...
  for (const auto &constraint : constraints) {
    ...
    constraint_pool_data.insert(constraint_pool_data.end(), constraint->begin(),
                                constraint->end());
  ...
  if ((additional_data.mapping_update_flags_inner_faces |
    ...
    if (task_info.scheme == internal::MatrixFreeFunctions::TaskInfo::none) {
      ...
      std::fill(hard_vectorization_boundary.begin(),
                hard_vectorization_boundary.end(), true);
    }
    ...
    if (additional_data.mapping_update_flags_inner_faces != update_default)
      internal::MatrixFreeFunctions::collect_faces_vectorization(
          face_setup.inner_faces, hard_vectorization_boundary,
          task_info.face_partition_data, face_info.faces,
    ...
    internal::MatrixFreeFunctions::collect_faces_vectorization(
        face_setup.boundary_faces, hard_vectorization_boundary,
        task_info.boundary_partition_data, face_info.faces,
        dof_info_hp.cell_active_fe_index);

    internal::MatrixFreeFunctions::collect_faces_vectorization(
        face_setup.refinement_edge_faces, hard_vectorization_boundary,
        task_info.refinement_edge_face_partition_data, face_info.faces,
        dof_info_hp.cell_active_fe_index);
    ...
    internal::MatrixFreeFunctions::collect_faces_vectorization(
        face_setup.inner_ghost_faces, hard_vectorization_boundary,
        task_info.ghost_face_partition_data, face_info.faces,
        dof_info_hp.cell_active_fe_index);

    cell_level_index.resize(
        cell_level_index.size() +
        VectorizedArrayType::size() *
            (task_info.refinement_edge_face_partition_data.back() -
    ...
    for (unsigned int f = 0; f < task_info.ghost_face_partition_data.back();
      ...
      for (unsigned int v = 0; v < VectorizedArrayType::size() &&
        ...
        TableIndices<3> index(
            face_info.faces[f].cells_interior[v] / VectorizedArrayType::size(),
    ...
    for (auto &di : dof_info)
      di.compute_tight_partitioners(
          ...
          *(task_info.cell_partition_data.end() - 2) *
              VectorizedArrayType::size(),
          VectorizedArrayType::size(), face_setup.inner_faces,
          face_setup.inner_ghost_faces, face_setup.refinement_edge_faces,
...
#ifdef DEAL_II_WITH_MPI
  {
    ...
    is_non_buffering_sm_supported = Utilities::MPI::logical_and(
        is_non_buffering_sm_supported, task_info.communicator);
    ...
    if (is_non_buffering_sm_supported) {
      ...
      cell_indices_contiguous_sm[2] = [&]() {
        ...
        for (unsigned int cell = 0;
          ...
          for (unsigned int v = 0;
               v < this->n_active_entries_per_cell_batch(cell); ++v) {
            ...
            if (cell < n_cell_batches()) {
              ...
              if (ptr != sm_procs.end()) {
                ...
                cells_shared_ghosts[sm_rank].emplace_back(index,
                                                          global_cell_index);
              } else {
        ...
        std::sort(
            cells_locally_owned.begin(), cells_locally_owned.end(),
        ...
        Utilities::MPI::ConsensusAlgorithms::Selector<
            ...
            std::vector<unsigned int>>()
            .run(targets, create_request, answer_request, process_answer,
                 communicator_sm);
      ...
      for (unsigned int i = 0; i < 2; ++i) {
        ...
        for (unsigned int face_index = 0;
             face_index < cell_indices_contiguous_sm[i].size(); ++face_index) {
          const unsigned int cell_index =
      ...
      for (auto &di : dof_info)
        di.compute_shared_memory_contiguous_indices(cell_indices_contiguous_sm);
    }
  }
#endif
...
void MatrixFree<dim, Number, VectorizedArrayType>::print_memory_consumption(
...
template <int dim, typename Number, typename VectorizedArrayType>
void MatrixFree<dim, Number, VectorizedArrayType>::print(
