...
public:
  ActualCellWork(MFWorkerInterface **worker_pointer,
                 const unsigned int partition, const TaskInfo &task_info)
      : worker(nullptr), worker_pointer(worker_pointer), partition(partition),
        task_info(task_info) {}

  ActualCellWork(MFWorkerInterface &worker, const unsigned int partition,
                 const TaskInfo &task_info)
      : worker(&worker), worker_pointer(nullptr), partition(partition),
        task_info(task_info) {}
  ...
  void operator()() const {
    ...
    if (task_info.face_partition_data.empty() == false) {
      ...
      if (task_info.refinement_edge_face_partition_data.empty() == false)
        used_worker->refinement_edge_face(partition);
    }
  }
...
public:
  CellWork(MFWorkerInterface &worker, const unsigned int partition,
           const TaskInfo &task_info, const bool is_blocked)
      : dummy(nullptr), work(worker, partition, task_info),
...
public:
  PartitionWork(MFWorkerInterface &function_in, const unsigned int partition_in,
                const TaskInfo &task_info_in, const bool is_blocked_in = false)
      : dummy(nullptr), function(function_in), partition(partition_in),
        task_info(task_info_in), is_blocked(is_blocked_in) {}
  ...
  tbb::task *execute() override {
    ...
    for (unsigned int j = 0; j < evens; ++j) {
      worker[j] = new (root->allocate_child())
          CellWork(function, task_info.partition_row_index[partition] + 2 * j,
                   task_info, false);
      if (j > 0) {
      ...
      if (j < evens - 1) {
        blocked_worker[j] = new (worker[j]->allocate_child()) CellWork(
            function, task_info.partition_row_index[partition] + 2 * j + 1,
            task_info, true);
      } else {
        if (odds == evens) {
...
class CellWork {
public:
  CellWork(MFWorkerInterface &worker_in, const TaskInfo &task_info_in,
...
class PartitionWork : public tbb::task {
public:
  PartitionWork(MFWorkerInterface &worker_in, const unsigned int partition_in,
                const TaskInfo &task_info_in, const bool is_blocked_in)
      : dummy(nullptr), worker(worker_in), partition(partition_in),
        task_info(task_info_in), is_blocked(is_blocked_in) {}

  tbb::task *execute() override {
    const unsigned int n_chunks =
        (task_info.cell_partition_data[partition + 1] -
         task_info.cell_partition_data[partition] + task_info.block_size - 1) /
...
void TaskInfo::loop(MFWorkerInterface &funct) const {
  ...
  if (scheme != none) {
    ...
    if (scheme == partition_partition && evens > 0) {
      ...
      std::vector<partition::PartitionWork *> blocked_worker(n_blocked_workers);
    ...
    } else // end of partition-partition, start of partition-color
    {
      ...
      if (odds > 0) {
        ...
        MPICommunication *worker_compr =
            new (root->allocate_child()) MPICommunication(funct, true);
        worker_compr->set_ref_count(1);
        for (unsigned int part = 0; part < partition_row_index.size() - 1;
             part++) {
          if (part == 0)
  ...
  // the beginning/end of the second part
  {
    for (unsigned int part = 0; part < partition_row_index.size() - 2; ++part) {
      ...
      for (unsigned int i = partition_row_index[part];
           i < partition_row_index[part + 1]; ++i) {
        ...
        if (face_partition_data.empty() == false) {
          ...
          if (boundary_partition_data[i + 1] > boundary_partition_data[i])
            funct.boundary(i);
          if (refinement_edge_face_partition_data.empty() == false &&
              refinement_edge_face_partition_data[i + 1] >
                  refinement_edge_face_partition_data[i])
            funct.refinement_edge_face(i);
        }
...
void TaskInfo::clear() {
  ...
  refinement_edge_face_partition_data.clear();
  refinement_edge_face_partition_data_hp.clear();
  refinement_edge_face_partition_data_hp_ptr.clear();
  evens = 0;
  odds = 0;
  n_blocked_workers = 0;
  n_workers = 0;
...
std::size_t TaskInfo::memory_consumption() const {
  return (sizeof(*this) +
...
void TaskInfo::make_boundary_cells_divisible(
    ...
    while (fillup_needed > 0 && bound_index < boundary_cells.size()) {
      if (next_free_slot < boundary_cells[bound_index]) {
        ...
        // next boundary cell
          ...
          for (unsigned int j = next_free_slot; j < boundary_cells[bound_index];
               ++j)
            new_boundary_cells.push_back(j);
...
void TaskInfo::create_blocks_serial(
    const std::vector<unsigned int> &cells_with_comm,
    const unsigned int dofs_per_cell, const bool categories_are_hp,
  ...
  temporary_numbering.reserve(n_active_cells + (n_lanes - 1) * n_categories);
  ...
  for (unsigned int j = 0; j < n_categories; ++j) {
    ...
    temporary_numbering.insert(temporary_numbering.end(), other_cells.begin(),
                               other_cells.end());
  ...
  if (n_procs == 1) {
    ...
    batch_order.insert(batch_order.begin() + comm_begin,
                       batch_order_comm.begin(), batch_order_comm.end());
  ...
  if (cell_vectorization_categories.empty() == false) {
    ...
    for (unsigned int i = 0; i < n_ghost_cells; ++i)
      used_categories.insert(cell_vectorization_categories[i + n_active_cells]);
    ...
    for (unsigned int i = 0; i < counters.size(); ++i)
      if (counters[i] % n_lanes != 0)
        for (unsigned int j = counters[i] % n_lanes; j < n_lanes; ++j)
          tight_category_map_ghost.emplace_back(
              std::array<unsigned int, 2>{{i, numbers::invalid_unsigned_int}});

    std::sort(tight_category_map_ghost.begin(), tight_category_map_ghost.end());
  }
  ...
  const unsigned int n_ghost_batches =
      ((tight_category_map_ghost.empty() ? n_ghost_cells
       ...
       n_lanes - 1) /
      n_lanes;
  incompletely_filled_vectorization.resize(n_cell_batches + n_ghost_batches);
  ...
  for (unsigned int block = 0; block < blocks.size() - 1; ++block) {
    ...
    for (unsigned int k = blocks[block]; k < blocks[block + 1]; k += grain_size)
    ...
    for (unsigned int k = blocks[block]; k < blocks[block + 1]; ++k) {
      ...
      unsigned int j = 0;
      for (; j < n_lanes &&
             temporary_numbering[pos + j] != numbers::invalid_unsigned_int;
           ++j)
  ...
  if (tight_category_map_ghost.empty()) {
    ...
    for (unsigned int k = 0, ptr = 0; k < n_ghost_batches;
      ...
      for (; j < n_lanes && (ptr + j < tight_category_map_ghost.size()) &&
...
void TaskInfo::initial_setup_blocks_tasks(
  ...
  if (n_procs > 1) {
    const unsigned int n_macro_boundary_cells =
        ...
        vectorization_length;
    cell_partition_data.push_back((n_cell_batches - n_macro_boundary_cells) /
                                  2);
...
void TaskInfo::guess_block_size(const unsigned int dofs_per_cell) {
  ...
  if (block_size == 0) {
    ...
    block_size = n_active_cells /
                 (MultithreadInfo::n_threads() * 16 * vectorization_length);
    ...
    block_size = 1 << static_cast<unsigned int>(std::log2(block_size + 1));
  }
  if (block_size > n_active_cells)
    block_size = std::max(1U, n_active_cells);
}

void TaskInfo::make_thread_graph_partition_color(
    DynamicSparsityPattern &connectivity_large,
    std::vector<unsigned int> &renumbering,
    std::vector<unsigned char> &irregular_cells, const bool) {
  ...
  make_connectivity_cells_to_blocks(irregular_cells, connectivity_large,
                                    connectivity);
  ...
  make_partitioning(connectivity, cluster_size, cell_partition, partition_list,
                    partition_size, partition);

  // Color the cells within each partition
  make_coloring_within_partitions_pre_blocked(
      connectivity, partition, cell_partition, partition_list, partition_size,
  ...
  for (unsigned int block = 0; block < n_blocks; ++block) {
    ...
    for (unsigned int mcell = mcell_start;
         mcell < std::min(mcell_start + block_size, n_cell_batches); ++mcell) {
  ...
  unsigned int block_size_last = n_cell_batches - block_size * (n_blocks - 1);
...
void TaskInfo::make_thread_graph(
    const std::vector<unsigned int> &cell_active_fe_index,
    DynamicSparsityPattern &connectivity,
    std::vector<unsigned int> &renumbering,
    std::vector<unsigned char> &irregular_cells, const bool hp_bool) {
  const unsigned int n_cell_batches = *(cell_partition_data.end() - 2);
  if (n_cell_batches == 0)
    return;
  ...
  make_connectivity_cells_to_blocks(irregular_cells, connectivity,
                                    connectivity_blocks);
...

  {
    ...
    make_partitioning_within_partitions_post_blocked(
        ...
        cell_partition, partition_list, partition_size, partition_2layers_list,
        irregular_cells);
  } else if (scheme == partition_color || scheme == color) {
    make_coloring_within_partitions_pre_blocked(
        connectivity_blocks, partition, cell_partition, partition_list,
        partition_size, partition_2layers_list);
  }
...
void TaskInfo::make_thread_graph_partition_partition(
    const std::vector<unsigned int> &cell_active_fe_index,
    DynamicSparsityPattern &connectivity,
    std::vector<unsigned int> &renumbering,
    std::vector<unsigned char> &irregular_cells, const bool hp_bool) {
  ...
  make_partitioning_within_partitions_post_blocked(
      connectivity, cell_active_fe_index, partition, cluster_size, hp_bool,
      cell_partition, partition_list, partition_size, partition_partition_list,
      irregular_cells);
...
void TaskInfo::make_connectivity_cells_to_blocks(
  ...
  for (unsigned int i = 0, mcell = 0; i < n_blocks; ++i) {
    for (unsigned int c = 0;
         ...
         ++c, ++mcell) {
      unsigned int ncomp = (irregular_cells[mcell] > 0) ? irregular_cells[mcell]
                                                        : vectorization_length;
...
void TaskInfo::make_partitioning_within_partitions_post_blocked(
    const DynamicSparsityPattern &connectivity,
    const std::vector<unsigned int> &cell_active_fe_index,
    const unsigned int partition, const unsigned int cluster_size,
    const bool hp_bool, const std::vector<unsigned int> &cell_partition,
...

  {
    ...
    for (unsigned int part = 0; part < partition; ++part) {
      ...
      while (work) {
        if (neighbor_list.empty()) {
          ...
          for (unsigned int j = start_up; j < partition_size[part + 1]; ++j)
            if (cell_partition[partition_list[j]] == part &&
              ...
              cell_partition_l2[partition_list[start_up]] = partition_l2;
              neighbor_neighbor_list.push_back(partition_list[start_up]);
              partition_partition_list[counter++] = partition_list[start_up];
              ++start_up;
              break;
            }
        } else {
          ...
          for (const unsigned int neighbor : neighbor_list) {
            Assert(cell_partition[neighbor] == part, ExcInternalError());
            ...
            for (; neighbor_it != end_it; ++neighbor_it) {
              if (cell_partition[neighbor_it->column()] == part &&
                  cell_partition_l2[neighbor_it->column()] ==
                      numbers::invalid_unsigned_int) {
                cell_partition_l2[neighbor_it->column()] = partition_l2;
                neighbor_neighbor_list.push_back(neighbor_it->column());
                partition_partition_list[counter++] = neighbor_it->column();
                ++partition_counter;
              }
            }
          }
        ...
        if (partition_counter > 0) {
              ...
              index = index_before;
          {
            ...
            missing_macros = 0;
            std::vector<unsigned int> remaining_per_cell_batch(max_fe_index +
                                                               1);
              ...
              for (cell = counter - partition_counter; cell < counter; ++cell) {
                renumbering_fe_index
                    [cell_active_fe_index.empty()
                         ? 0
                         : cell_active_fe_index[partition_partition_list[cell]]]
            ...
            while (missing_macros > 0 || filled == false) {
              ...
              // fill up with.
              DynamicSparsityPattern::iterator neighbor = connectivity.begin(
                                                   additional),
                                               end =
              ...
              for (; neighbor != end; ++neighbor) {
                if (cell_partition[neighbor->column()] == part &&
                  ...
                  if (hp_bool == true)
                    this_index = cell_active_fe_index.empty()
                                     ? 0
                  ...
                  if (missing_macros > 0 ||
                    ...
                    if (hp_bool == true)
                      renumbering_fe_index[this_index].push_back(
                          neighbor->column());
                    partition_partition_list[counter] = neighbor->column();
                    ++counter;
                    ++partition_counter;
                    if (remaining_per_cell_batch[this_index] == 0 &&
                        missing_macros > 0)
                      missing_macros--;
                    remaining_per_cell_batch[this_index]++;
                    if (remaining_per_cell_batch[this_index] ==
                        vectorization_length) {
                      remaining_per_cell_batch[this_index] = 0;
                    }
                    if (missing_macros == 0) {
                      filled = true;
                      for (unsigned int fe_ind = 0; fe_ind < max_fe_index + 1;
                           ++fe_ind)
                        if (remaining_per_cell_batch[fe_ind] != 0)
                          filled = false;
                    }
                    if (filled == true)
                      break;
            ...
            if (hp_bool == true) {
              ...
              for (unsigned int j = 0; j < max_fe_index + 1; ++j) {
                for (const unsigned int jj : renumbering_fe_index[j])
                  renumbering[cell++] = jj;
                if (renumbering_fe_index[j].size() % vectorization_length != 0)
                  irregular_cells[renumbering_fe_index[j].size() /
                      ...
                      renumbering_fe_index[j].size() % vectorization_length;
                n_cell_batches_before += (renumbering_fe_index[j].size() +
                                          vectorization_length - 1) /
                                         vectorization_length;
                renumbering_fe_index[j].resize(0);
              }
            } else {
              n_cell_batches_before += partition_counter / vectorization_length;
      ...
      partition_row_index[part + 1] = partition_row_index[part] + partition_l2;
    }
  }
  if (hp_bool == true) {
...
void TaskInfo::make_coloring_within_partitions_pre_blocked(
    const DynamicSparsityPattern &connectivity, const unsigned int partition,
  ...
  for (unsigned int part = 0; part < partition; ++part) {
    ...
    for (unsigned int k = partition_size[part]; k < partition_size[part + 1];
      ...
      for (unsigned int j = 0; j <= n_neighbors; ++j)
        color_finder[j] = true;
      DynamicSparsityPattern::iterator neighbor = connectivity.begin(cell),
...
void TaskInfo::make_partitioning(const DynamicSparsityPattern &connectivity,
  ...
  while (work) {
    ...
    while (neighbor_list.size() > 0) {
      ...
      for (const unsigned int cell : neighbor_list) {
        Assert(cell_partition[cell] == partition - 1, ExcInternalError());
...
void TaskInfo::update_task_info(const unsigned int partition) {
  evens = (partition + 1) / 2;
  odds = partition / 2;
  ...
  for (unsigned int part = 0; part < partition; ++part) {
    ...
    partition_n_workers[part] = partition_evens[part] + partition_odds[part] -
                                partition_n_blocked_workers[part];
  }
}
} // namespace MatrixFreeFunctions
...
internal::MatrixFreeFunctions::TaskInfo::print_memory_statistics<std::ostream>(
    std::ostream &, const std::size_t) const;
template void internal::MatrixFreeFunctions::TaskInfo::print_memory_statistics<
