...
public:
  ...
  /// Runs the body of the work on refinement edge faces specified by
  /// MatrixFree::loop
  virtual void refinement_edge_face(const unsigned int range_index) = 0;
};
...
struct TaskInfo {
   ...
   * according to the renumbering returned by this function.
   */
  void initial_setup_blocks_tasks(
   ...
   * @param hp_bool Defines whether we are in hp-mode or not
   */
  void make_thread_graph_partition_partition(
   ...
   * @param hp_bool Defines whether we are in hp-mode or not
   */
  void make_thread_graph(const std::vector<unsigned int> &cell_active_fe_index,
                         DynamicSparsityPattern &connectivity,
                         std::vector<unsigned int> &renumbering,
                         std::vector<unsigned char> &irregular_cells,
                         const bool hp_bool);
   ...
   */
  void make_partitioning(const DynamicSparsityPattern &connectivity,
  ...
  /**
   * Like refinement_edge_face_partition_data but with precomputed
   * subranges for each active fe index. The start and end point of a
   * partition is given by refinement_edge_face_partition_data_hp_ptr.
   */
  std::vector<unsigned int> refinement_edge_face_partition_data_hp;

  /**
   * Pointers within refinement_edge_face_partition_data_hp, indicating
   * the start and end of a partition.
   */
  std::vector<unsigned int> refinement_edge_face_partition_data_hp_ptr;
