 ...
 */
template <int dim> struct FaceSetup {
  FaceSetup();
   ...
   * later).
   */
  FaceToCellTopology<1> create_face(
      ...
      const unsigned int number_cell_exterior, const bool is_mixed_mesh);
...
void collect_faces_vectorization(
...
template <int dim> FaceSetup<dim>::FaceSetup() : use_active_cells(true) {}

template <int dim>
void FaceSetup<dim>::initialize(
    const dealii::Triangulation<dim> &triangulation,
    const unsigned int mg_level, const bool hold_all_faces_to_owned_cells,
    const bool build_inner_faces,
    std::vector<std::pair<unsigned int, unsigned int>> &cell_levels) {
  ...
  if (triangulation.locally_owned_subdomain() !=
    ...
    for (auto &inner_face : inner_faces_at_proc_boundary) {
      ...
      int ierr =
          MPI_Sendrecv(&mysize, 1, MPI_UNSIGNED, inner_face.first,
                       600 + my_domain, &othersize, 1, MPI_UNSIGNED,
                       inner_face.first, 600 + inner_face.first, comm, &status);
      AssertThrowMPI(ierr);
      AssertDimension(mysize, othersize);
      mysize = inner_face.second.n_hanging_faces_smaller_subdomain;
      ierr =
          MPI_Sendrecv(&mysize, 1, MPI_UNSIGNED, inner_face.first,
                       700 + my_domain, &othersize, 1, MPI_UNSIGNED,
                       inner_face.first, 700 + inner_face.first, comm, &status);
      AssertThrowMPI(ierr);
      AssertDimension(mysize, othersize);
      mysize = inner_face.second.n_hanging_faces_larger_subdomain;
      ierr =
          MPI_Sendrecv(&mysize, 1, MPI_UNSIGNED, inner_face.first,
                       800 + my_domain, &othersize, 1, MPI_UNSIGNED,
                       inner_face.first, 800 + inner_face.first, comm, &status);
      AssertThrowMPI(ierr);
      AssertDimension(mysize, othersize);
#endif
      ...
      for (unsigned int i = 0; i < other_range.size(); ++i)
        other_range[i] =
            std::make_tuple(inner_face.second.shared_faces[i].second,
                            inner_face.second.shared_faces[i].first, i);
          ...
          if (std::get<0>(other_range[i]) ==
            ...
            if (count > 0) {
              for (unsigned int k = 0; k <= count; ++k) {
                Assert(
                    inner_face.second
                            .shared_faces[std::get<2>(other_range[i - 1])]
                            .second ==
                        inner_face.second
                            .shared_faces[std::get<2>(other_range[i - 1 - k])]
                            .second,
                    ExcInternalError());
      ...
      if (split_index < n_faces_lower_proc)
        ...
        split_index = n_total_faces_at_proc_boundary - n_faces_higher_proc -
                      n_faces_lower_proc;

        // make sure the splitting is consistent between both sides
#if defined(DEAL_II_WITH_MPI) && defined(DEBUG)
      ierr =
          MPI_Sendrecv(&split_index, 1, MPI_UNSIGNED, inner_face.first,
                       900 + my_domain, &othersize, 1, MPI_UNSIGNED,
                       inner_face.first, 900 + inner_face.first, comm, &status);
      AssertThrowMPI(ierr);
      AssertDimension(split_index, othersize);
      ierr = MPI_Sendrecv(&n_faces_lower_proc, 1, MPI_UNSIGNED,
                          inner_face.first, 1000 + my_domain, &othersize, 1,
                          MPI_UNSIGNED, inner_face.first,
                          1000 + inner_face.first, comm, &status);
      AssertThrowMPI(ierr);
      AssertDimension(n_faces_lower_proc, othersize);
      ierr = MPI_Sendrecv(&n_faces_higher_proc, 1, MPI_UNSIGNED,
                          inner_face.first, 1100 + my_domain, &othersize, 1,
                          MPI_UNSIGNED, inner_face.first,
                          1100 + inner_face.first, comm, &status);
      AssertThrowMPI(ierr);
      ...
      AssertIndexRange(split_index, inner_face.second.shared_faces.size() + 1 -
      ...
      // inside the domain in case of multigrid separately
        ...
        if ((id1 == id2 &&
          ...
          if (use_active_cells)
            ...
            add_to_ghost =
                (dcell->level_subdomain_id() != neighbor->level_subdomain_id());
            ...
            if (neighbor->has_children())
              for (unsigned int s = 0; s < dcell->face(f)->n_children(); ++s)
                if (dcell->at_boundary(f)) {
                  if (dcell->periodic_neighbor_child_on_subface(f, s)
                          ->subdomain_id() != dcell->subdomain_id())
                    add_to_ghost = true;
                } else {
          ...
          if (use_active_cells && neighbor->has_children())
            for (unsigned int s = 0; s < dcell->face(f)->n_children(); ++s) {
              typename dealii::Triangulation<dim>::cell_iterator
                  neighbor_child =
                      dcell->at_boundary(f)
                          ? dcell->periodic_neighbor_child_on_subface(f, s)
              ...
              if (neighbor_child->subdomain_id() != dcell->subdomain_id())
                ghost_cells.insert(std::pair<unsigned int, unsigned int>(
                    neighbor_child->level(), neighbor_child->index()));
            }
          else
            ghost_cells.insert(std::pair<unsigned int, unsigned int>(
...
template <int dim>
void FaceSetup<dim>::generate_faces(
    const dealii::Triangulation<dim> &triangulation,
    const std::vector<std::pair<unsigned int, unsigned int>> &cell_levels,
    TaskInfo &task_info) {
  ...
  for (unsigned int partition = 0;
          ...
          // interior face, including faces over periodic boundaries
            ...
            if (use_active_cells && neighbor->has_children()) {
              ...
              for (unsigned int c = 0; c < n_children; ++c) {
                ...
                if (neigh_domain != dcell->subdomain_id() ||
                  ...
                  std::pair<unsigned int, unsigned int> level_index(
                      neighbor_c->level(), neighbor_c->index());
                  if (face_is_owned[child_face_index] ==
                      FaceCategory::locally_active_done_here) {
                    ++inner_counter;
                    inner_faces.push_back(
                        create_face(neighbor_face_no, neighbor_c,
                                    map_to_vectorized[level_index], dcell, cell,
                                    is_mixed_mesh));
                  } else if (face_is_owned[child_face_index] ==
                                 FaceCategory::ghosted ||
                             face_is_owned[dcell->face(f)->index()] ==
                                 FaceCategory::ghosted) {
                    inner_ghost_faces.push_back(
                        create_face(neighbor_face_no, neighbor_c,
                                    map_to_vectorized[level_index], dcell, cell,
                                    is_mixed_mesh));
                  } else
              ...
              const types::subdomain_id neigh_domain =
                  use_active_cells ? neighbor->subdomain_id()
                                   : neighbor->level_subdomain_id();
              const unsigned int face_index = dcell->face(f)->index();
              if (neigh_domain != my_domain || face_visited[face_index] == 1) {
                std::pair<unsigned int, unsigned int> level_index(
                    neighbor->level(), neighbor->index());
                if (face_is_owned[dcell->face(f)->index()] ==
                    FaceCategory::locally_active_done_here) {
                  ...
                  inner_faces.push_back(create_face(
                      f, dcell, cell, neighbor, map_to_vectorized[level_index],
                      is_mixed_mesh));
                } else if (face_is_owned[face_index] == FaceCategory::ghosted) {
                ...
                if (dcell->has_periodic_neighbor(f))
                  face_visited[neighbor
                                   ->face(dcell->periodic_neighbor_face_no(f))
                                   ->index()] = 1;
              }
              if (face_is_owned[face_index] ==
                  FaceCategory::multigrid_refinement_edge) {
                std::pair<unsigned int, unsigned int> level_index_re(
                    neighbor->level(), neighbor->index());
                refinement_edge_faces.push_back(create_face(
                    f, dcell, cell, neighbor, map_to_vectorized[level_index_re],
                    is_mixed_mesh));
                ++refinement_edge_counter;
              }
            }
          }
        }
    ...
    task_info.refinement_edge_face_partition_data[partition + 1] =
        task_info.refinement_edge_face_partition_data[partition] +
        refinement_edge_counter;
  }
...
template <int dim>
FaceToCellTopology<1> FaceSetup<dim>::create_face(
    const unsigned int face_no,
    const typename dealii::Triangulation<dim>::cell_iterator &cell,
    const unsigned int number_cell_interior,
    const typename dealii::Triangulation<dim>::cell_iterator &neighbor,
  ...
  info.face_type = is_mixed_mesh ? (cell->face(face_no)->reference_cell() !=
                                    ReferenceCells::get_hypercube<dim - 1>())
                                 : 0;
  ...
  if (cell->has_periodic_neighbor(face_no)) {
    ...
    if (cell->level() > neighbor->level() && exterior_face_orientation > 0) {
      ...
      const auto face_reference_cell = cell->face(face_no)->reference_cell();
      info.subface_index =
 ...
 */
inline bool compare_faces_for_vectorization(
    const FaceToCellTopology<1> &face1, const FaceToCellTopology<1> &face2,
 ...
 */
template <int length> struct FaceComparator {
...
void collect_faces_vectorization(
  ...
  for (unsigned int partition = 0; partition < face_partition_data.size() - 1;
       ++partition) {
    ...
    for (unsigned int face = face_start; face < face_end; ++face) {
      for (auto &face_type : faces_type) {
        ...
        if (compare_faces_for_vectorization(
                faces_in[face], faces_in[face_type[0]], active_fe_indices,
                vectorization_width)) {
    ...
    for (const auto &face_type : faces_type) {
      ...
      for (unsigned int f = 0; f < no_faces; ++f)
        if (faces_in[face_type[f]].cells_interior[0] % vectorization_width ==
            0) {
